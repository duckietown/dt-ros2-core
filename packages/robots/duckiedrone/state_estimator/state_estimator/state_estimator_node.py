...
class StateEstimatorNode(Node):
    """Estimate drone state from IMU, altitude, pose, and optical-flow odometry."""
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("ir_throttled", False)
        self.declare_parameter("imu_throttled", False)
        self.declare_parameter("optical_flow_throttled", False)
        self.declare_parameter("camera_pose_throttled", False)
        self.declare_parameter("sdim", 1)
        self.declare_parameter("student_ukf", False)
        ...
        if primary != "ema":
            raise NotImplementedError(
                f"state_estimator primary '{primary}' is not ported yet; use 'ema'."
            )
    ...
    def range_cb(self, msg: Range) -> None:
        """Apply EMA smoothing to the tilt-corrected altitude estimate."""
        ...
        smoothed_altitude = (1.0 - alpha) * current_altitude + alpha * previous_altitude
    ...
    def twist_cb(self, msg: Odometry) -> None:
        """Apply EMA smoothing to optical-flow linear velocity estimates."""
        ...
        velocity.x = self._near_zero((1.0 - alpha) * velocity.x + alpha * new_velocity.x)
        velocity.y = self._near_zero((1.0 - alpha) * velocity.y + alpha * new_velocity.y)
        velocity.z = self._near_zero((1.0 - alpha) * velocity.z + alpha * new_velocity.z)
        self.state_msg.twist.twist.angular = msg.twist.twist.angular

    def state_callback(self) -> None:
        """Republish the current EMA state once the core inputs are available."""
    ...
    def _roll_pitch_from_orientation(self) -> tuple[float, float]:
        """Convert the current quaternion orientation to roll and pitch."""
    ...
    def _near_zero(value: float, epsilon: float = 1e-6) -> float:
        """Zero out tiny floating-point values for readability/stability."""
