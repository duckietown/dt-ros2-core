...
from opencv_apps.msg import Flow, FlowArrayStamped
from rclpy.node import Node
from sensor_msgs.msg import Range


class OpticalFlowNode(Node):
    """Convert motion vectors into projected optical-flow odometry."""
    ...
    def __init__(self) -> None:
        ...
        self._range = 1.0
        self._scale = self._range / float(self.get_parameter("base_homography_height").value)

        self.pub_odometry = self.create_publisher(Odometry, "~/visual_odometry", 1)
        self.pub_seg_list = self.create_publisher(SegmentList, "~/lineseglist_out", 1)
        self.pub_debug_raw_odometry = self.create_publisher(Odometry, "~/debug/raw_odometry", 1)

        self.create_subscription(FlowArrayStamped, "~/motion_vectors", self.cb_motion_vectors, 1)
    ...
    def cb_motion_vectors(self, msg: FlowArrayStamped) -> None:
        """Repackage motion vectors as a SegmentList for ground projection."""
        ...
        for flow in msg.flow:
            ...
            segment = Segment(
                points=[
                    ...
                    PointMsg(
                        x=flow.point.x + flow.velocity.x,
                        y=flow.point.y + flow.velocity.y,
                    ),
                ]
            )
            segment_list.segments.append(segment)

        if self.pub_debug_raw_odometry.get_subscription_count() > 0 and msg.flow:
            ...
            odometry_msg.twist.twist.linear.x = float(
                np.mean([flow.velocity.x for flow in msg.flow]) * flow_scale * self._range
            )
            odometry_msg.twist.twist.linear.y = float(
                np.mean([flow.velocity.y for flow in msg.flow]) * flow_scale * self._range
            )
