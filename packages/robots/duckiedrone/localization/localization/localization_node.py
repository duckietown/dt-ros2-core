...
from .localization_helper.helper import (
    PROB_THRESHOLD,
...
CAMERA_HEIGHT = 640
METER_TO_PIXEL = (
    (float(MAP_PIXEL_WIDTH) / MAP_REAL_WIDTH)
    + (float(MAP_PIXEL_HEIGHT) / MAP_REAL_HEIGHT)
) / 2.0
...
class LocalizationNode(Node):
    """Monte-Carlo localization from camera features."""

    def __init__(self) -> None:
        """Initialize state, estimator, and ROS interfaces."""
    ...
    def image_callback(self, msg: CompressedImage) -> None:
        """Update the localization estimate from a compressed image."""
        try:
            curr_img = self.bridge.compressed_imgmsg_to_cv2(
                msg,
        ...
        if self.locate_position:
            ...
            if curr_kp is not None and curr_des is not None and curr_kp:
    ...
    def state_callback(self, msg: Odometry) -> None:
        """Update altitude and tilt state from odometry."""
    ...
    def reset_callback(
        ...
        """Reset localization and allow a fresh initialization."""
...
def _is_almost_equal(x: float, y: float) -> bool:
    epsilon = 1e-8
