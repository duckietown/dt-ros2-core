...
class SLAMNode(Node):
    ...
    def __init__(self) -> None:
        """Initialize SLAM state and ROS interfaces."""
    ...
    def image_callback(self, msg: CompressedImage) -> None:
        """Process a new compressed camera frame."""
        try:
        ...
        if self.locate_position:
            ...
            if curr_kp is not None and len(curr_kp) > 0:
                if self.first_locate:
                    ...
                    pose, weight = self.estimator.run(
                        self.z,
                        self.prev_kp,
                        self.prev_des,
                        curr_kp,
    ...
    def state_callback(self, msg: Odometry) -> None:
        """Update altitude and attitude estimates from odometry."""
    ...
    def update_position(self, new_pose: Sequence[float]) -> None:
        """Blend a new SLAM pose estimate into the current state."""
        self.pos = [
            self.hybrid_alpha * new_pose[0]
            + (1.0 - self.hybrid_alpha) * self.pos[0],
            self.hybrid_alpha * new_pose[1]
            + (1.0 - self.hybrid_alpha) * self.pos[1],
            self.z,
            self.alpha_yaw * new_pose[3]
    ...
    def _broadcast_transform(self) -> None:
        """Broadcast the current world-to-base transform."""
